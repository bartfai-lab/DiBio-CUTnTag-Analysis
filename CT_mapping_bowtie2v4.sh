...
while getopts ":n:g:p:d:b:x:q:m:a:" opt; do
  case $opt in
    n)
      filename=$OPTARG
      ;;
...
#defines filename with -n, genome location with -g, filter quality with -q, project path with -p (location of fastq folder containing fastq files)
#optional arguments: duplicate removal on/off with -d, bigWig generation on/off with -b, exclude reads with -m or -a (default removes Pf3D7 mitochondrium and apicoplast reads)
...
#Run bowtie2
bowtie2 --end-to-end --very-sensitive --no-mixed --no-discordant --phred33 -I 10 -X 700 -p 8 -x ${genome} -1 ${projPath}/fastq/${filename}_R1.fastq.gz -2 ${projPath}/fastq/${filename}_R2.fastq.gz -S ${projPath}/${filename}/alignment/sam/${filename}_bowtie2.sam &> ${projPath}/${filename}/alignment/sam/bowtie2_summary/${filename}_bowtie2.txt
...
echo -e "$(samtools view -c $projPath/alignment/sam/${filename}_bowtie2.sam)\treads before filtering" >> $projPath/alignment/${filename2}.filter_report.txt
...
echo "filtering out ${API}"
		...
		#concatenate header and sam file
